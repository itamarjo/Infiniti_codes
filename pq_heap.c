...
int WrapBefore(const void *data1, const void *data2, void *param)
{
	before_t *wrap = (before_t *)param;

	return(wrap->is_before(*(void **)data1, *(void **)data2,wrap->param));
}
					...
					void *param)
{
	...
	new->heap = HeapCreate(WrapBefore, sizeof(void *), &new->before);
	if (0 == new)
	{
		free(new);
		return(0);
...
static int is_match_wrap(const void *data, void *param)
{
